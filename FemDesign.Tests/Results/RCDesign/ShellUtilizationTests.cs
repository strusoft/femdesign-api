...
namespace FemDesign.Results
{
    ...
    public class ShellUtilizationTests
    {
        ...
        public void Parse()
        {
            ...
            var (resultLines, headers, results) = UtilTestMethods.GetCsvParseData<RCShellUtilization>(modelPath);


            // Check parsed data
            Assert.IsTrue(results.First().GetType() == typeof(RCShellUtilization), $"{typeof(RCShellUtilization).Name} should be parsed");
            Assert.IsTrue(results.Last().GetType() == typeof(RCShellUtilization), $"{typeof(RCShellUtilization).Name} should be parsed");
            ...
            foreach (var header in headers)
            {
                ...
                {
                    var headerMatch = RCShellUtilization.HeaderExpression.Match(line);
