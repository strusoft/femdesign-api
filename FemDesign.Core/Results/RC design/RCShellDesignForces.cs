...
namespace FemDesign.Results
{
    ...
    public class RCShellDesignForces : IResult
    {
        ...
        internal static Regex HeaderExpression
        {
            get
            {
                return new Regex(@"(?'max'Max. of load combinations, Shell, Design forces)|(?'type'Shell, Design forces), ((?'loadcasetype'[\w\s]+)? - )?Load (?'casecomb'case|comb.+): (?'casename'[ -#%'-;=?A-\ufffd]{1,79})|Shell\t|\[.*\]");
            }
        }
