...
namespace FemDesign.Results
{
    ...
    public class RCShellShearUtilization : IResult
    {
        ...
        internal static Regex HeaderExpression
        {
            get
            {
                return new Regex(@"(?'Cmax'Max\. of load combinations, Shell, Shear utilization)|(?'Gmax'Max\. of load groups, Shell, Shear utilization)|(?'type'Shell, Shear utilization), ((?'loadcasetype'[\w\s]+)? - )?Load (?'casecomb'case|comb.+): (?'casename'[ -#%'-;=?A-\ufffd]{1,79})|^Shell\tMax\.\t(Combination\t)?vRd,Max\tvRd,c\tvRd,s|^\[.*\]");
            }
        }
