...
namespace FemDesign.Results
{
    ...
    public partial class ShellStress : IResult
    {
        ...
        internal static Regex HeaderExpression
        {
            get
            {
                return new Regex(@"^(?'type'Shells, Stresses), (?'side'top|bottom|membrane) ?(?'extract'\(Extract\))?, ((?'loadcasetype'[\w\s\-]+)? - )?Load (?'casecomb'case|comb\.+): (?'casename'[ -#%'-;=?A-\ufffd]{1,79})$|^Shell\t(Max\.\t)?Elem\tNode\t\u03c3 x'\t\u03c3 y'\t\u03c4 x'y'\t\u03c4 x'z'\t\u03c4 y'z'\t\u03c3 vm\t\u03c3 1\t\u03c3 2\t\u03b1\t(Case|Comb\.)|^\[.*\]");
            }
        }
