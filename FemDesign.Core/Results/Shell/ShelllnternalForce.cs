...
namespace FemDesign.Results
{
    ...
    public partial class ShellInternalForce : IResult
    {
        ...
        internal ShellInternalForce(string id, int elementId, string nodeId, double mx, double my, double mxy, double nx, double ny, double nxy, double txz, double tyz, string resultCase)
        {
            Id = id;
            ElementId = elementId;
            NodeId = nodeId;
        ...
        internal static Regex IdentificationExpression
        {
            get
            {
                return new Regex(@"^(?'type'Shells), (?'result'Internal forces) ?(?'extract'\(Extract\))?, (?'LoadCaseType'Ultimate|Frequent|Quasi-permanent|Characteristic).+Load (?'casecomb'case|comb.+): (?'casename'[ -#%'-;=?A-\ufffd]{1,79})$");
            }
        }
        ...
        internal static Regex HeaderExpression
        {
            get
            {
                return new Regex(@"^(?'type'Shells), (?'result'Internal forces) ?(?'extract'\(Extract\))?, (?'LoadCaseType'Ultimate|Frequent|Quasi-permanent|Characteristic).+Load (?'casecomb'case|comb.+): (?'casename'[ -#%'-;=?A-\ufffd]{1,79})$|ID\tElem\tNode\tMx'\tMy'\tMx'y'\tNx'\tNy'\tNx'y'\tTx'z'\tTy'z'.*|ID\tMax\..*|\[.*\]");
            }
        }
